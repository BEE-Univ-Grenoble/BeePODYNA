...
com_plot <- function(x, ...) {
  ...
  plot(x = com[[2]][[1]]$size, y = com[[2]][[2]]$size, type = "n",
       xlab = com[[2]][[1]]$label, ylab = com[[2]][[2]]$label)

  gray = gray.colors(length(com[[2]][[1]]$size), start =1 , end = 0.1, gamma = 2.2)
  for(i in 1:length(com[[2]][[1]]$size)){
    segments(x0 = com[[2]][[1]]$size[i],y0 = com[[2]][[2]]$size[i] ,
             x1 = com[[2]][[1]]$size[i+1], y1 = com[[2]][[2]]$size[i+1],
             col = gray[i])
    text(x = com[[2]][[1]]$size[i], y = com[[2]][[2]]$size[i],
           label = com[[2]][[2]]$time[i],col = gray[i], pos = 3)
  }
